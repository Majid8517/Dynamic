...
class FpnCombine(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor]):
        ...
        softplus_weights = F.softplus(self.edge_weights.to(dtype=dtype))
        norm_weights = softplus_weights / (softplus_weights.sum() + 1e-4)

        out = torch.stack(nodes, dim=-1) * norm_weights
